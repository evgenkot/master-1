...
def load_data(file_path='mnist.pkl.gz'):
    """
    Загружает данные MNIST из указанного файла.

    :param file_path: Путь к файлу с данными MNIST (по умолчанию 'mnist.pkl.gz').
    :return: Кортеж с обучающими, валидационными и тестовыми данными.
    """
    with gzip.open(file_path, 'rb') as f:  # открываем сжатый файл gzip в двоичном режиме
        training_data, validation_data, test_data = pickle.load(f, encoding='latin1')  # загружаем таблицы из файла
    return (training_data, validation_data, test_data)


def load_data_wrapper(file_path='mnist.pkl.gz'):
    """
    Загружает данные MNIST и преобразует их в нужный формат.

    :param file_path: Путь к файлу с данными MNIST (по умолчанию 'mnist.pkl.gz').
    :return: Кортеж с обучающими, валидационными и тестовыми данными в нужном формате.
    """
    tr_d, va_d, te_d = load_data(file_path)  # инициализация наборов данных в формате MNIST
